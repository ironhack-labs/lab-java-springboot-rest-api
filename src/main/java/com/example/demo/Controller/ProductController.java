...
public class ProductController {
    ...
    @PutMapping("/{name}")
    public ResponseEntity<?> updateProductById(@RequestHeader("API-Key") String api_Key,@PathVariable String name,@RequestBody Product product){
        if(!isValidApiKey(api_Key)){
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Invalid api key");
        }
    ...
    @DeleteMapping("/{name}")
    public ResponseEntity<?> deleteProductById(@RequestHeader("API-Key") String api_Key,@PathVariable String name){
        if(!isValidApiKey(api_Key)){
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Invalid api key");
        }
